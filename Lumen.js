export default function bot({ history: h, memory: m }) {
  ...
  if (m.s === "H") {
    ...
    if (step < HL) {
      ...
      if (IL) nm.or = "S"; else nm.or = opp === "D" ? "L" : "S";
      m.hs = step + 1;
      return [my, m];
    } else {
      m.s = "T";
...
function dn(h, m) {
  ...
  if (opp === "D") { nm.cd = (nm.cd||0)+1; nm.cc = 0; }
  else if (opp === "C") { nm.cc = (nm.cc||0)+1; nm.cd = 0; }
  ...
  if (r >= 6 && !nm.cp) {
    ...
    let match = true;
    for (let i = 0; i < 6; i++) {
      if (h[i].opponent !== H[i]) { match = false; break; }
    }
    if (match) { nm.cp = true; nm.cm = true; }
  }
